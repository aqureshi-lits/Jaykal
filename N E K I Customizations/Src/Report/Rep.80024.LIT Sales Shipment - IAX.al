report 80024 "LIT Sales Shipment - IAX"
{
    DefaultLayout = RDLC;
    RDLCLayout = './Src/Layout/Sales Shipment - IAX.rdlc';
    Caption = 'Posted Sales Shipment - IAX';
    ...
    dataset
    {
        dataitem(DataItem1; "Sales Shipment Header")
        {
            ...
            column(CityandCountry; CompanyInformation.City + ' -' + CompanyInformation."Country/Region Code")
            {
            }
            ...
            dataitem(DataItem9; "Sales Shipment Line")
            {
                ...
                column(QtytoShip_SalesLine; "Qty. Shipped Not Invoiced")
                {
                }
                dataitem(DataItem13; "Reservation Entry")
                {
                    DataItemLink = "Item No." = FIELD("No."),
                                   "Variant Code" = FIELD("Variant Code"),
                                   "Source ID" = FIELD("Document No.");
                    DataItemTableView = WHERE("Qty. to Handle (Base)" = FILTER(<> 0));
                    ...
                    column(ExpirationDate_ItemLedgerEntry; ItemExpiry)
                    {
                    }
                    column(Quantity_ItemLedgerEntry; FORMAT(-"Qty. to Handle (Base)") + (' KG'))
                    {
                    }
                    ...
                    begin
                        ...
                        IF ILE.FINDFIRST THEN BEGIN
                            ItemExpiry := ILE."Expiration Date";
                        END;
                    end;
                }
