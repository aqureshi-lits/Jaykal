report 80043 "LIT Statement Of Accounts 2"
{
    DefaultLayout = RDLC;
    RDLCLayout = './Src/Layout/Statement Of Accounts 2.rdl';
                        ...
                        dataitem(DtldCustLedgEntries; "Detailed Cust. Ledg. Entry")
                        {
                            ...
                            begin
                                if SkipReversedUnapplied(DtldCustLedgEntries) or (Amount = 0) then
                                    CurrReport.Skip();
                                if DtldCustLedgEntries."Document Type" = DtldCustLedgEntries."Document Type"::Invoice then
                                    invoiceAmount += DtldCustLedgEntries.Amount;
                                if DtldCustLedgEntries."Document Type" = DtldCustLedgEntries."Document Type"::Payment then
                                    PaymentAmount += DtldCustLedgEntries.Amount;
                                if DtldCustLedgEntries."Document Type" = DtldCustLedgEntries."Document Type"::Refund then
                                    RefundAmount += DtldCustLedgEntries.Amount;
                                PayTotal := PaymentAmount + RefundAmount;
                                RemainingAmount := 0;
                                PrintLine := true;
                                case "Entry Type" of
